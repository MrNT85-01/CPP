...
function cpp_admin_assets($hook) {
    ...
    // Inline styles removed - should be in admin.css
        ...
        window.cpp_init_media_uploader = function() {
            ...
            jQuery("body").off("click.cppuploader", ".cpp-upload-btn").on("click.cppuploader", ".cpp-upload-btn", function(e) {
                ...
                var inputId = button.data("input-id") || button.siblings("input[type=\"text\"]").attr("id");
                 if (!inputId) { input_field = button.prev("input[type=\'text\']"); }
                 else { input_field = jQuery("#" + inputId); }
                var preview_img_container = button.closest("td, .cpp-image-uploader-wrapper, .form-table tr").find(".cpp-image-preview");
                if (!input_field.length) { console.error("CPP Uploader: Could not find target input field."); return; }

                mediaUploader = wp.media({ title: "'.esc_js(__('انتخاب یا آپلود تصویر', 'cpp-full')).'", button: { text: "'.esc_js(__('استفاده از این تصویر', 'cpp-full')).'" }, multiple: false });
    ...
    if ($count > 0) {
        ...
        foreach ($menu as $key => $value) {
            if ($menu[$key][2] == $menu_slug) {
                ...
                // Add bubble to the Orders submenu item
                 global $submenu;
...
function cpp_handle_admin_actions() {
    ...
    // افزودن دسته‌بندی جدید (Non-AJAX)
    if (isset($_POST['cpp_add_category']) && $current_page === 'custom-prices-categories') {
        ...
        if (!empty($name)) {
             ...
             if ($inserted) $message = 'category_added';
             else error_log("CPP DB Error adding category: " . $wpdb->last_error);
        }
    ...
    if (isset($_POST['cpp_add_product']) && $current_page === 'custom-prices-products') {
        ...
        $data = [
            ...
            'image_url'    => isset($_POST['image_url']) ? esc_url_raw(wp_unslash($_POST['image_url'])) : '',
            'created'      => current_time('mysql', 1), 'last_updated_at' => current_time('mysql', 1)
        ];
        ...
        if (!empty($data['name']) && !empty($data['cat_id'])) {
             ...
             } else error_log("CPP DB Error adding product: " . $wpdb->last_error);
        }
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id']) && isset($_GET['_wpnonce'])) {
        ...
        $deleted = false; $db_table = ''; $nonce_action = ''; $message_success = ''; $message_failed = '';
        if ($current_page == 'custom-prices-categories') { $nonce_action = 'cpp_delete_cat_' . $id; $db_table = CPP_DB_CATEGORIES; $message_success = 'category_deleted'; $message_failed = 'category_delete_failed'; }
        elseif ($current_page == 'custom-prices-products') { $nonce_action = 'cpp_delete_product_' . $id; $db_table = CPP_DB_PRODUCTS; $message_success = 'product_deleted'; $message_failed = 'product_delete_failed'; }
        elseif ($current_page == 'custom-prices-orders') { $nonce_action = 'cpp_delete_order_' . $id; $db_table = CPP_DB_ORDERS; $message_success = 'order_deleted'; $message_failed = 'order_delete_failed'; }
        ...
        } else wp_die(__('Invalid delete request or security check failed.', 'cpp-full'));
...
// ۶. توابع AJAX برای محصولات (فرم ویرایش)
add_action('wp_ajax_cpp_fetch_product_edit_form', 'cpp_fetch_product_edit_form');
function cpp_fetch_product_edit_form() {
    if (!isset($_GET['security']) || !wp_verify_nonce(sanitize_text_field($_GET['security']), 'cpp_admin_nonce')) wp_send_json_error(['message' => __('Security check failed.', 'cpp-full')], 403);
    $product_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
    if (!$product_id) wp_send_json_error(['message' => __('Invalid Product ID.', 'cpp-full')], 400);
    global $wpdb; $product_exists = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM " . CPP_DB_PRODUCTS . " WHERE id = %d", $product_id));
    if(!$product_exists) wp_send_json_error(['message' => __('Product not found.', 'cpp-full')], 404);
    ob_start(); include CPP_TEMPLATES_DIR . 'product-edit.php'; $html = ob_get_clean();
    wp_send_json_success(['html' => $html]); wp_die();
}
add_action('wp_ajax_cpp_handle_edit_product_ajax', 'cpp_handle_edit_product_ajax');
function cpp_handle_edit_product_ajax() {
    global $wpdb;
    if (!isset($_POST['cpp_edit_product_nonce']) || !wp_verify_nonce(sanitize_text_field($_POST['cpp_edit_product_nonce']), 'cpp_edit_product_action')) wp_send_json_error(['message' => __('Security check failed.', 'cpp-full')], 403);
    $product_id = isset($_POST['product_id']) ? intval($_POST['product_id']) : 0;
    if (!$product_id) wp_send_json_error(['message' => __('Invalid Product ID.', 'cpp-full')], 400);
    $data = [
        'cat_id' => isset($_POST['cat_id']) ? intval($_POST['cat_id']) : 0, 'name' => isset($_POST['name']) ? sanitize_text_field(wp_unslash($_POST['name'])) : '',
        'price' => isset($_POST['price']) ? sanitize_text_field(wp_unslash($_POST['price'])) : '', 'min_price' => isset($_POST['min_price']) ? sanitize_text_field(wp_unslash($_POST['min_price'])) : '', 'max_price' => isset($_POST['max_price']) ? sanitize_text_field(wp_unslash($_POST['max_price'])) : '',
        'product_type' => isset($_POST['product_type']) ? sanitize_text_field(wp_unslash($_POST['product_type'])) : '', 'unit' => isset($_POST['unit']) ? sanitize_text_field(wp_unslash($_POST['unit'])) : '', 'load_location'=> isset($_POST['load_location']) ? sanitize_text_field(wp_unslash($_POST['load_location'])) : '',
        'is_active' => isset($_POST['is_active']) ? intval($_POST['is_active']) : 0, 'description' => isset($_POST['description']) ? wp_kses_post(wp_unslash($_POST['description'])) : '', 'image_url' => isset($_POST['image_url']) ? esc_url_raw(wp_unslash($_POST['image_url'])) : '',
        'last_updated_at' => current_time('mysql', 1)
    ];
    if (empty($data['name']) || empty($data['cat_id'])) wp_send_json_error(['message' => __('Product name and category are required.', 'cpp-full')], 400);
    ...
    if ($updated !== false) {
        ...
        wp_send_json_success(['message' => __('Product updated successfully.', 'cpp-full')]);
    } else wp_send_json_error(['message' => __('Error updating product in database.', 'cpp-full') . ' ' . $wpdb->last_error], 500);
    wp_die();
}

// ۷. توابع AJAX برای دسته‌بندی‌ها (فرم ویرایش)
...
function cpp_fetch_category_edit_form() {
    ...
    if (!$cat_id) wp_send_json_error(['message' => __('Invalid Category ID.', 'cpp-full')], 400);
    global $wpdb; $cat_exists = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM " . CPP_DB_CATEGORIES . " WHERE id = %d", $cat_id));
    if(!$cat_exists) wp_send_json_error(['message' => __('Category not found.', 'cpp-full')], 404);
    ob_start(); include CPP_TEMPLATES_DIR . 'category-edit.php'; $html = ob_get_clean();
    wp_send_json_success(['html' => $html]); wp_die();
}
add_action('wp_ajax_cpp_handle_edit_category_ajax', 'cpp_handle_edit_category_ajax');
function cpp_handle_edit_category_ajax() {
    global $wpdb;
    if (!isset($_POST['cpp_edit_cat_nonce']) || !wp_verify_nonce(sanitize_text_field($_POST['cpp_edit_cat_nonce']), 'cpp_edit_cat_action')) wp_send_json_error(['message' => __('Security check failed.', 'cpp-full')], 403);
    $cat_id = isset($_POST['category_id']) ? intval($_POST['category_id']) : 0;
    if (!$cat_id) wp_send_json_error(['message' => __('Invalid Category ID.', 'cpp-full')], 400);
    $name = isset($_POST['name']) ? sanitize_text_field(wp_unslash($_POST['name'])) : '';
    $slug = isset($_POST['slug']) ? sanitize_title(wp_unslash($_POST['slug'])) : '';
    $image_url = isset($_POST['image_url']) ? esc_url_raw(wp_unslash($_POST['image_url'])) : '';
    if (empty($name)) wp_send_json_error(['message' => __('Category name is required.', 'cpp-full')], 400);
    ...
    if ($updated !== false) wp_send_json_success(['message' => __('Category updated successfully.', 'cpp-full')]);
    else wp_send_json_error(['message' => __('Error updating category.', 'cpp-full') . ' ' . $wpdb->last_error], 500);
    wp_die();
}


// --- شروع تغییر: اصلاح تابع ویرایش سریع تکی ---
...
function cpp_quick_update() {
    ...
    $value = null;
    if (in_array($field, ['admin_note', 'description'])) $value = wp_kses_post($value_raw);
    elseif ($field === 'is_active') $value = intval($value_raw);
    elseif ($field === 'slug') $value = sanitize_title($value_raw);
    // price field (base price) sanitized as text
    elseif ($field === 'price') $value = sanitize_text_field($value_raw);
    else $value = sanitize_text_field($value_raw); // Default
    ...
    // Explicitly disallow price range fields here
    if (in_array($field, ['min_price', 'max_price']) && $table_type === 'products') wp_send_json_error(['message' => 'از اکشن مخصوص بازه قیمت استفاده کنید.'], 400);
    if (!in_array($field, $allowed_fields)) wp_send_json_error(['message' => 'فیلد نامعتبر: ' . esc_html($field)], 400);
    ...
    if ($table_type === 'products') {
        $data_to_update['last_updated_at'] = current_time('mysql', 1); // GMT
        if ($field === 'price') {
            $old_data = $wpdb->get_row($wpdb->prepare("SELECT price FROM " . $table . " WHERE id = %d", $id));
            if ($old_data && $old_data->price != $value) CPP_Core::save_price_history($id, $value, 'price');
        }
    ...
    if ($updated === false) wp_send_json_error(['message' => 'خطای دیتابیس: ' . $wpdb->last_error], 500);
    wp_send_json_success($response_data);
    wp_die();
}

// تابع AJAX جدید برای ویرایش سریع بازه قیمت
add_action('wp_ajax_cpp_quick_update_price_range', 'cpp_quick_update_price_range');
function cpp_quick_update_price_range() {
    check_ajax_referer('cpp_admin_nonce', 'security');
    global $wpdb;

    $id    = isset($_POST['id']) ? intval($_POST['id']) : 0;
    $table_type = isset($_POST['table_type']) ? sanitize_key($_POST['table_type']) : '';
    $min_price_raw = isset($_POST['min_price']) ? wp_unslash($_POST['min_price']) : null;
    $max_price_raw = isset($_POST['max_price']) ? wp_unslash($_POST['max_price']) : null;

    if (!$id || $table_type !== 'products' || $min_price_raw === null || $max_price_raw === null) wp_send_json_error(['message' => 'اطلاعات بازه قیمت ناقص است.'], 400);

    $min_price = sanitize_text_field($min_price_raw);
    $max_price = sanitize_text_field($max_price_raw);

    $data_to_update = ['min_price' => $min_price, 'max_price' => $max_price, 'last_updated_at' => current_time('mysql', 1)]; // GMT
    $response_data = ['message' => __('بازه قیمت به‌روز شد.', 'cpp-full')];

    $old_data = $wpdb->get_row($wpdb->prepare("SELECT min_price, max_price FROM " . CPP_DB_PRODUCTS . " WHERE id = %d", $id));

    $updated = $wpdb->update(CPP_DB_PRODUCTS, $data_to_update, ['id' => $id]);

    if ($updated === false) wp_send_json_error(['message' => 'خطای دیتابیس (بازه قیمت): ' . $wpdb->last_error], 500);

    // Save history if values changed
    if ($old_data) {
        if ($old_data->min_price != $min_price) CPP_Core::save_price_history($id, $min_price, 'min_price');
        if ($old_data->max_price != $max_price) CPP_Core::save_price_history($id, $max_price, 'max_price');
    }
    $response_data['new_time'] = date_i18n('Y/m/d H:i:s', current_time('timestamp')); // Local time
    wp_send_json_success($response_data);
    wp_die();
}
// --- پایان تغییر ---
...
function cpp_enqueue_styles_elementor() { if (!wp_style_is('cpp-front-css', 'enqueued')) cpp_front_assets(); }
...
function cpp_ajax_test_email() {
    check_ajax_referer('cpp_admin_nonce', 'security');
    $capability = get_option('cpp_admin_capability', 'manage_options');
    if (!current_user_can($capability)) wp_send_json_error(['log' => 'Error: No permission.'], 403);
    $log = "--- Starting Email Test ---\nTime: " . current_time('mysql', 1) . "\n";
    $to = get_option('cpp_admin_email', get_option('admin_email'));
    if (empty($to) || !is_email($to)) { $log .= "Error: Invalid admin email.\n"; wp_send_json_error(['log' => $log]); return; }
    $log .= "Sending test email to: " . esc_html($to) . "\n";
    $subject = 'Test Email from CPP Plugin'; $message = '<p>This is a test email.</p>'; $headers = ['Content-Type: text/html; charset=UTF-8'];
    $mail_error = null; $mail_failed_hook = function ($wp_error) use (&$mail_error) { $mail_error = $wp_error; };
    add_action('wp_mail_failed', $mail_failed_hook, 10, 1);
    $sent = wp_mail($to, $subject, $message, $headers);
    remove_action('wp_mail_failed', $mail_failed_hook, 10);
    if ($sent) { $log .= "Success: wp_mail() executed.\nCheck inbox/spam.\n"; wp_send_json_success(['log' => $log]); }
    else { $log .= "Error: wp_mail() failed.\n"; if ($mail_error instanceof WP_Error) $log .= "Details: " . esc_html(implode(', ', $mail_error->get_error_messages())) . "\n"; else $log .= "No specific WP_Error. Check server config/SMTP plugin.\n"; wp_send_json_error(['log' => $log]); }
    wp_die();
}
...
function cpp_ajax_test_sms() {
    ...
    $service = get_option('cpp_sms_service'); $apiKey = get_option('cpp_sms_api_key'); $sender = get_option('cpp_sms_sender');
    $adminPhone = get_option('cpp_admin_phone'); $pattern_code = get_option('cpp_sms_pattern_code');
    if (empty($service) || $service !== 'ippanel') { $log .= "Error: IPPanel Service inactive.\n"; wp_send_json_error(['log' => $log]); return; }
    if (empty($apiKey)) { $log .= "Error: API Key missing.\n"; wp_send_json_error(['log' => $log]); return; }
    if (empty($sender)) { $log .= "Error: Sender missing.\n"; wp_send_json_error(['log' => $log]); return; }
    ...
    $test_variables = ['product_name'=>'Test SMS', 'customer_name'=>'Admin', 'phone'=>$adminPhone, 'qty'=>'1', 'unit'=>'Test', 'load_location'=>'Test', 'note'=>'CPP Test'];
    $log .= "Vars: " . json_encode($test_variables, JSON_UNESCAPED_UNICODE) . "\n";
    $data = ['code'=>$pattern_code, 'sender'=>$sender, 'recipient'=>$adminPhone, 'variable'=>$test_variables];
    $body = json_encode($data); $headers = ['Content-Type'=>'application/json', 'apikey'=>$apiKey];
    $args = ['body'=>$body, 'headers'=>$headers, 'method'=>'POST', 'timeout'=>25];
    $log .= "Sending POST...\n"; $response = wp_remote_post($url, $args);
    if (is_wp_error($response)) { $error_message = $response->get_error_message(); $log .= "WP HTTP Error: " . esc_html($error_message) . "\n"; wp_send_json_error(['log' => $log], 500); }
    else {
        $http_code = wp_remote_retrieve_response_code($response); $response_body = wp_remote_retrieve_body($response);
        ...
        if ($http_code >= 200 && $http_code < 300) {
            ...
            } else { $api_error = ($result && isset($result->status->message)) ? $result->status->message : 'Unknown API Logic Error'; $log .= "API Error: " . esc_html($api_error) . "\nCheck pattern/vars.\n"; wp_send_json_error(['log' => $log]); }
        } else { $error_detail = $safe_body; $result = json_decode($response_body); if ($result && isset($result->status->message)) $error_detail = $result->status->message; elseif ($result && isset($result->errorMessage)) $error_detail = $result->errorMessage; elseif ($result && isset($result->message)) $error_detail = $result->message; $log .= "HTTP Error.\nDetail: " . esc_html($error_detail) . "\nCheck API Key/Sender/Pattern.\n"; wp_send_json_error(['log' => $log], $http_code); }
    }
    wp_die();
}
